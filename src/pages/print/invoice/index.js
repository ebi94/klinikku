...
const InvoicePrint = () => {
  ...
  const { treatmentId, nurseId } = router.query;
  console.log(treatmentId, nurseId);
  const [data, setData] = useState({});
  const [loading, setLoading] = useState(true);
  // const data = fakeDataInvoice;
  // ** Hook
  const theme = useTheme();

  const fetchDataDetail = async () => {
    const res = await detailInvoice(treatmentId, nurseId);
