...
import { editClinic, detailClinic, listClinic } from "src/services/clinics";
...
const schema = yup.object().shape({
  name: yup
    .string()
    .min(3, (obj) => showErrors("Nama Klinik", obj.value.length, obj.min))
    .required("Kolom ini harus diisi !"),
  code: yup
    .string()
    .min(3, (obj) => showErrors("Kode Klinik", obj.value.length, obj.min))
    .required("Kolom ini harus diisi !"),
...
const ClinicEdit = () => {
  ...
  const setDefaultValue = (data) => {
    const selectedProvince = listProvince.filter(
      (item) => item?.id === data.province_id
    );
    console.log("selectedProvince", selectedProvince);
    const mappingDataFacility =
      data?.facility.length > 0 ? data?.facility.map((item) => item?.name) : [];
    setValue("name", data?.name ?? "");
    setValue("code", data?.code ?? "");
    setValue("complete_address", data?.complete_address ?? "");
    setValue("province_id", selectedProvince[0]);
    setValue("city_district_id", data?.city_district_id);
    setValue("subdistrict_id", data?.subdistrict_id);
    setValue("village_district_id", data?.village_subdistrict_id);
    setValue("phone", data?.phone?.[0] ?? "");
    setValue("phone_nd", data?.phone?.[1] ?? "");
    setValue("latitude", data?.latitude ?? "");
    setValue("longitude", data?.longitude ?? "");
    setValue("map_links", data?.map_links ?? "");
    // setValue("weekdays_open", data?.operational_hours?.weekdays?.open ?? "");
    // setValue("weekdays_close", data?.operational_hours?.weekdays?.close ?? "");
    // setValue("weekend_open", data?.operational_hours?.weekend?.open ?? "");
    // setValue("weekend_close", data?.operational_hours?.weekend?.close ?? "");
    setValue("facility", mappingDataFacility);
  };
  ...
  const mappingDataProvince = (data) => {
  ...
  const mappingDataCityDistrict = (data) => {
    const mappingDataList = data.map((item) => {
      return {
        id: item?.city_district_id,
        value: item?.city_district_id,
        label: item?.name,
        ...item,
      };
    });
  ...
  const mappingDataSubDistrict = (data) => {
    const mappingDataList = data.map((item) => {
      return {
  ...
  const mappingDataVillageDistrict = (data) => {
    const mappingDataList = data.map((item) => {
      return {
        id: item?.village_subdistrict_id,
        value: item?.village_subdistrict_id,
        label: item?.name,
        ...item,
      };
    });
  ...
  const fetchListRegion = async (
    ...
    if (+res?.result?.status === 200) {
      ...
      if (region === "province") mappingDataProvince(data);
      if (region === "city") mappingDataCityDistrict(data);
      if (region === "subdistrict") mappingDataSubDistrict(data);
      if (region === "village") mappingDataVillageDistrict(data);
    } else {
      console.error(res);
    }
  };
  ...
  useEffect(() => {
    if (listProvince?.length > 0 && dataDetail) {
      const selectedProvince = listProvince.filter(
        (item) => +item?.id === +dataDetail.province_id
      );
      setValue("province_id", selectedProvince[0]);
      if (+dataDetail.province_id)
        fetchListRegion("city", +dataDetail.province_id);
    }
  }, [listProvince, dataDetail]);
  ...
  return (
    <DatePickerWrapper>
      <Grid
        ...
        spacing={6}
      >
        <PageHeader breadCrumbs={breadCrumbs} title="Edit Clinic" />
