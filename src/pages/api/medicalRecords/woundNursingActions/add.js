...
export default function handler(req, res) {
  ...
  if (req.method === "POST") {
    const { treatment_id, wound_nursing_actions } = req.body;

    if (!treatment_id || !wound_nursing_actions) {
      return res
        .status(400)
    ...
    const bodyReq = {
      treatment_id,
      wound_nursing_actions,
    };

    const requestOptions = {
      method: "POST",
      url: `${baseUrl}api/backoffice/customers/medical-record/wound-nursing-actions/create`,
      headers,
      data: bodyReq,
      redirect: "follow",
    };
